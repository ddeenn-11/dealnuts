...
- **On-device logo matching (shipped, wired into the capture flow):** TensorFlow.js + a pretrained MobileNet feature extractor, matching the captured photo against a curated reference set of **124 brand logos** (`src/assets/logos/`, sourced from Wikimedia Commons and tracked in `_manifest.json`) via cosine similarity of image embeddings. Runs automatically alongside OCR on every capture in `scanTag()` (`src/utils/ocr.js`); if it clears the confidence floor, its brand guess wins over the OCR text guess (`brandSource: 'logo'` vs `'text'`). **Color-invariant:** both the live query photo and the entire reference set are preprocessed into a shape-only Sobel edge/outline drawing (`src/utils/edges.js`) before embedding, so a logo matches by its outline, not its fill color — a red Nike swoosh and a black one embed to nearly the same vector.

  **Brands currently in the logo library (124):** Acne Studios, Adidas, Alexander McQueen, American Eagle, Ami Paris, Armani, Ashworth, Asics, Balenciaga, Bally, Banana Republic, Bath & Body Works, Boss, Bottega Veneta, Breitling, Burberry, C.P. Company, CASETiFY, Callaway, Calvin Klein, Camper, Casio, Celine, Champion, Chevignon, Chloé, Chow Tai Fook, Citizen, Clarks, Club Monaco, Coach, Cole Haan, Columbia Sportswear, Converse, Crocs, DeLonghi, Descente, Diesel, Dior, Dolce & Gabbana, Dsquared2, Dunhill, Ecco, Etro, Fendi, Ferragamo, Furla, G2000, Gap, Geox, Givenchy, Gucci, Hamilton, Helly Hansen, J.Lindeberg, Jil Sander, Jimmy Choo, K-Swiss, Kate Spade New York, Kenzo, Kipling, Lacoste, Le Creuset, Lego, Levi's, Loewe, Longchamp, Longines, Loro Piana, Lukfook Jewellery, Lululemon, Maison Margiela, Marimekko, Marithé François Girbaud, Marni, MAX&Co., MaxMara, Michael Kors, Miss Sixty, Miu Miu, Molton Brown, Moncler, Montblanc, New Balance, Nike, Oakley, Off-White, Palladium, Pandora, Polo Ralph Lauren, Prada, Puma, Ray-Ban, Replay, Saint Laurent, Salomon, Samsonite, Sandro Paris, Seiko, Skechers, Stone Island, Swarovski, Swatch, Tag Heuer, TaylorMade, The North Face, Thermos, Timberland, Tissot, Tod's, Tommy Hilfiger, Tory Burch, TSL, Tumi, Ugg, Under Armour, UNIQLO, Vans, Versace, Victorinox, Vilebrequin, Woolrich, Zegna, Zwilling.

  **Targeted but not yet sourced** (56 more, per `_manifest.json`): brands Wikimedia Commons had no clean logo file for — e.g. Valentino, Vivienne Westwood, Marc Jacobs, Fila, Christian Louboutin, Stella McCartney, Brunello Cucinelli, Y-3, Thom Browne, Manolo Blahnik, Roger Vivier, Paul Smith, Arc'teryx, Hoka, Barbour, Mammut, The Body Shop, National Geographic, MLB, Theory, Maje, Repetto (full list in `_manifest.json`). Re-run the fetch script periodically in case Wikimedia gains a clean source for any of these — nothing here is a transient failure to retry, all 56 are genuine misses.
...
- **On-device OCR auto-scan** runs the moment a photo is attached: best-effort extraction of brand, price, category/subcategory, size, and color from the tag text (plus a dominant-pixel-color fallback, sampled from the center-cropped photo, when no color word is found). Every field it fills stays editable and is only prefilled if the user hasn't already typed something there — it never silently overwrites a manual entry.
- **On-device logo matching** runs in parallel with OCR on the same photo: matches against a 124-brand reference library using color-invariant (outline-based) image embeddings, and wins over the OCR text guess for Brand when it clears the confidence floor.
...
- **Logo matching is shipped and wired in** (`scanTag()` in `src/utils/ocr.js` calls `matchLogo()` alongside Tesseract on every capture; its brand guess wins over OCR text when confident). Still open: it loads TensorFlow.js + MobileNet (a real, sizeable client-side model) on first photo capture rather than being lazy-loaded only when OCR's own brand guess comes back empty — worth revisiting if first-scan load time on slow store wifi turns out to be a problem in practice. The `MIN_CONFIDENCE = 0.55` threshold is also still a guess, not empirically tuned against real photographed logos.
...
14. On-device brand-logo matching via TensorFlow.js/MobileNet, wired into the capture flow alongside OCR (124-logo reference library)
15. Fixed OCR brand-detection regex rejecting brand names ending in digits (Dsquared2, G2000, Y-3); fixed dominant-color detection missing gray/silver items
16. Made logo matching color-invariant: both the query photo and all 124 reference logos are preprocessed into shape-only outlines (Sobel edge detection) before embedding, so fill color no longer affects the match
